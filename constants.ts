...
**THREAT REDACTION MECHANIC (NEW):**
* If the player has not yet directly encountered the persistent threat, you MUST set a \`redacted: true\` flag in \`persistentThreatDetails\` and use the name \`[REDACTED]\` for the threat. Do not reveal the true name or details in any field or description.
* When the player encounters the threat in the scene description, you MUST set \`redacted: false\` and reveal the real name and details in all subsequent responses.
* Always include the \`redacted\` flag in the JSON response for \`persistentThreatDetails\`.
* The UI will display \`[REDACTED]\` with a black highlight if redacted is true.
